...
class SingleDownloadFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, return_callback, settings):
        super().__init__(parent)
        self.return_callback = return_callback
        self.settings = settings
        self.is_downloading = False
        self.music_folder = Path(settings.get("output_folder"))
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(folder_frame, text="Output:").pack(side="left", padx=5)
        self.folder_label = ctk.CTkLabel(folder_frame, text=str(self.music_folder), width=250)
        self.folder_label.pack(side="left", padx=5)

        ctk.CTkButton(folder_frame, text="Change", width=80, command=self.choose_folder).pack(side="left")
    ...
    def choose_folder(self):
        ...
        folder = filedialog.askdirectory(initialdir=str(self.music_folder))
        if folder:
            self.music_folder = Path(folder)
            self.folder_label.configure(text=str(self.music_folder))
            self.settings.set("output_folder", str(self.music_folder))
    ...
    def simulate_download(self):
        ...
        try:
            ...
            self.ui_queue.put({"type": "status", "text": "Downloading audio..."})
            self.ui_queue.put({"type": "progress", "value": 0.3})
            filename = yt_downloader(url, str(self.music_folder))

            self.ui_queue.put({"type": "status", "text": "Enhancing quality..."})
            self.ui_queue.put({"type": "progress", "value": 0.7})
            try:
            ...
            self.ui_queue.put({"type": "progress", "value": 1.0})
            self.ui_queue.put({"type": "status", "text": f"Download complete! Saved in {self.music_folder}."})
            self.ui_queue.put({"type": "complete"})

        except Exception as e:
            self.ui_queue.put({"type": "status", "text": f"Error: {str(e)}"})
            self.ui_queue.put({"type": "progress", "value": 0})

        finally:
            self.ui_queue.put({"type": "enable_check"})
...
class PlaylistDownloadFrame(ctk.CTkFrame):
    ...
    def list_playlist(self):
        """Start loading playlist items in the background."""
        url = self.url_entry.get().strip()
        if not url:
            self.playlist_text.delete("0.0", "end")
        ...
        self.playlist_text.delete("0.0", "end")
        self.playlist_text.insert("0.0", "Loading playlist items...")

        thread = threading.Thread(target=self.load_playlist, args=(url,))
        thread.daemon = True
        thread.start()

    def load_playlist(self, url):
        """Load playlist URLs and titles without blocking the GUI thread."""
        try:
            playlist_urls = playlist_lister(url)
            playlist_display = []
            total_items = len(playlist_urls)

            for i, video_url in enumerate(playlist_urls):
                self.ui_queue.put({"type": "status", "text": f"Loading title {i + 1}/{total_items}..."})
                try:
                ...
                self.ui_queue.put({"type": "progress", "value": (i + 1) / total_items})

            self.ui_queue.put({"type": "playlist_loaded", "urls": playlist_urls, "display_items": playlist_display})

        except Exception as e:
            self.ui_queue.put({"type": "playlist_error", "error": str(e)})
            self.ui_queue.put({"type": "progress", "value": 0})

        finally:
            self.ui_queue.put({"type": "enable_list"})
    ...
    def simulate_playlist_download(self):
        ...
        if not hasattr(self, 'playlist_urls') or not self.playlist_urls:
            self.ui_queue.put({"type": "status", "text": "Please list playlist items first"})
            self.ui_queue.put({"type": "enable_download"})
            return
        ...
        try:
            ...
            for i, url in enumerate(self.playlist_urls):
                self.ui_queue.put({"type": "status", "text": f"Downloading song {i+1}/{total_songs}..."})
                ...
                if i < total_songs - 1:
                    ...
                    self.ui_queue.put({"type": "status", "text": f"Waiting {delay}s before next download..."})
                    time.sleep(delay)

            self.ui_queue.put({"type": "status", "text": f"All songs downloaded! Saved in {self.music_folder}."})
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.single_download = SingleDownloadFrame(self, self.show_main_menu, self.settings)
        self.playlist_download = PlaylistDownloadFrame(self, self.show_main_menu, self.settings)
